# -*- coding: utf-8 -*-
"""
Created on Thu May 31 13:12:39 2018
...
L_force=MHSampling(ftd_force,[1]*(m+1),[0.1]*(m+1),15000)
a=[np.float64(0)]*(m+1)
...
def Gamma_force_parametric_model(T,force_estimate,m):
    def f_d_theta(theta):
        x=1
        mu0=theta[m]
        r=theta[m+1]
        ...
        for k in range(len(T)):
            i=T[k][0]
            j=T[k][1]
            t=(mu0+theta[i]+theta[j]+alpha*(abs(force_estimate[i]-force_estimate[j]+force_estimate[m])))/r
            x=T[k][3]
...
def gaussian_f_prior_gm(mu,mu0,r0,alpha0,v,v0,vr0,valpha0,m):
    mean=[0]*(m+3)
    for i in range(m):
        mean[i]=mu
    mean[m]=mu0
...
def Gamma_force_posterior(D,force_estimate,m,prior):#m number of teams for logit_parametric_model AND NOT number of parameters
    f1=Gamma_force_parametric_model(D,force_estimate,m)
    f2=prior
    def f(theta):
        x=f1(theta)*f2(theta)
        return x
...
ft_time=gaussian_f_prior_gm(mu,mu0,r0,alpha0,v,v0,vr0,valpha0,m)
ftd_time=Gamma_force_posterior(K1,a_force,m,ft_time)

B=[0]*(m+3)
B[m]=40
B[m+1]=8.2
B[m+2]=-2
L_time=MHSampling(ftd_time,B,[0.01]*(m+3),2000)
a=[np.float64(0)]*(m+3)
...
def gamma_force_prediction(theta_estimate,force_estimate,T):
    A=[]
    m=len(theta_estimate)
    for k in range(len(T)):
        i=T[k][0]
...
def Gamma_MSE(theta_estimate,force_estimate,T):
    MSE=0
    m=len(theta_estimate)
    n=len(T)
    for k in range(len(T)):
...
print(Gamma_MSE(a_time,a_force,K1),'0')
print(calculate_var_data(K1),'0')

print(Gamma_MSE(a_time,a_force,K2),'1')
print(calculate_var_data(K2),'1')
...
print(Gamma_MSE(a_time,a_force,K3),'2')
print(calculate_var_data(K3),'2')
...
plt_distribution(gamma_force_prediction(a_time,a_force,K2),20,90,55)
