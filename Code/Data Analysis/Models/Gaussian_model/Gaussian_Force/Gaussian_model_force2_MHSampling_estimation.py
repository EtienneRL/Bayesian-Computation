# -*- coding: utf-8 -*-
"""
Created on Sun May 27 13:07:43 2018
...
U=formatdatarow(I,[[1,[2014,2016,2017,2018]],[3,['promotion','regional']]])
J=indextoteam(U,4,7)
m=len(J)
...
L_force=MHSampling(ftd_force,[1]*(m+1),[0.5]*(m+1),5000)
a=[np.float64(0)]*(m+1)
...
def Gaussian_force_parametric_model2(T,force_estimate,m):
    def f_d_theta(theta):
        ...
        for k in range(len(T)):
            i=T[k][0]
            j=T[k][1]
            if abs(force_estimate[i]-force_estimate[j]+force_estimate[m])>theta[m+3]:
                fi=(1/(2*math.pi*(sigma**2)))**(1/2)*math.exp(-1/2*(((T[k][3]-(mu0+theta[i]+theta[j]+alpha*(abs(force_estimate[i]-force_estimate[j]+force_estimate[m]))))/sigma)**2))*100
            else:
...
def gaussian_force2_f_prior_gm(mu,mu0,sigma0,mu1,mu2,v,v0,vsigma0,v1,v2,m):
    mean=[0]*(m+4)
    for i in range(m):
        mean[i]=mu
    mean[m]=mu0
...
ft_time=gaussian_force2_f_prior_gm(mu,mu0,sigma0,mu1,mu2,v,v0,vsigma0,v1,v2,m)
...
#B[m+2]=-1
#B[m+3]=1
B=[-4.39668488e-01, -2.36576038e-01,  1.76069277e-01, -4.54800935e-01,
 -5.09785088e-01,  3.49615026e-02, -2.85820510e-01,  2.43734973e-01,
  1.53199374e-02 , 2.28825320e-01,  2.48743582e-01, -9.44754168e-02,
  3.52075656e-02,  7.42326002e-01,  5.45296389e-01,  3.37449302e-02,
  4.09487866e-02, -1.29539765e-01,  5.34726267e-02,  2.53732648e-02,
  3.15647431e-01,  2.53323254e-01, -1.09108530e-01, -2.32141431e-01,
  3.51116976e+01,  6.11929182e+00, -1.15870138e+00, -3.89439232e-03]
L_time=MHSampling(ftd_time,B,[0.1]*(m+4),5000)
a=[np.float64(0)]*(m+4)
...
def gaussian_force_prediction2(theta_estimate,force_estimate,T):
    ...
    for k in range(len(T)):
        i=T[k][0]
        j=T[k][1]
        if abs(force_estimate[i]-force_estimate[j]+force_estimate[m-4])>=theta_estimate[m-1]:
            tij=theta_estimate[m-4]+theta_estimate[i]+theta_estimate[j]+theta_estimate[m-2]*abs(force_estimate[i]-force_estimate[j]+force_estimate[m-4])
        else:
            tij=theta_estimate[m-4]+theta_estimate[i]+theta_estimate[j]
        A+=[tij]
    A=np.array(A)
    return A

def Gaussian_force2_MSE(theta_estimate,force_estimate,T):
    MSE=0
    m=len(theta_estimate)
    n=len(T)
    for k in range(len(T)):
        ...
        j=T[k][1]
        if abs(force_estimate[i]-force_estimate[j]+force_estimate[m-4])>=theta_estimate[m-1]:
            MSE+=(T[k][3]-(theta_estimate[m-4]+theta_estimate[i]+theta_estimate[j]+theta_estimate[m-2]*abs(force_estimate[i]-force_estimate[j]+force_estimate[m-4])))**2
        else:
            MSE+=(T[k][3]-(theta_estimate[m-4]+theta_estimate[i]+theta_estimate[j]))**2
    MSE=MSE/n
    return MSE
...
print(Gaussian_force2_MSE(a_time,a_force,K2))
...
print(Gaussian_force2_MSE(a_time,a_force,K3))
